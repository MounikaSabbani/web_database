	...
	if (mysqli_num_rows($result) > 0) {
		...
		if (mysqli_num_rows($info) > 0) {
			...
			if ($isadmin == 1 ) {
				...
				$_SESSION["isAuthor"] = 0;
				$_SESSION["isUser"] = 0;
				header('Location: '.$file_location.'admin_user.html');
			} elseif ($isauthor == 1) {
				$_SESSION["isAdmin"] = 0;
				$_SESSION["isAuthor"] = 1;
				$_SESSION["isUser"] = 0;
				header('Location: '.$file_location.'author_user.html');
			} else {
				$_SESSION["isAdmin"] = 0;
				$_SESSION["isAuthor"] = 0;
				$_SESSION["isUser"] = 1;
				header("Location: '.$file_location.'index.php?username=".$username);
				//?username=".$username
			}
		}
		else {
			...
			echo 'window.location.href = "'.$file_location.'demographic.html?username='.$username.'"';
			echo '</script>';
			mysqli_close($conn);					
		...
		echo 'window.location.href = "'.$file_location.'login.html"';
		echo '</script>';
		mysqli_close($conn);
	}
