  ...
  .status {
	...
	<head>
		...
		<title>Hi Admin</title>
		  <h2>Approve or Reject Stories</h2>
	</head>
	<main>        
  <div class="topnav">
...
$sql_update = "UPDATE STORIES SET story_status = '".$status."' WHERE Story_ID =".$StoryID;
$sql_email = "Select E.EMAIL as EMAIL from STORIES S, SIGNUP E WHERE S.username = E.username and S.Story_ID =".$StoryID;
    

if (mysqli_query($conn, $sql_update)) {
    echo "The status has been successfully updated for story - ";
    echo $StoryID;
    $author_email = mysqli_query($conn, $sql_email);
    $row =  mysqli_fetch_assoc($author_email);

         $to = $row ['EMAIL'];
         
         $subject = "Your Story has been ".$status;
         
         $message = "<b>Your Story has been ".$status."</b>";
         ...
         if( $retval == true ) {
            echo "<p> </p>";
            echo "Message sent successfully... <p> </p>";
         }else {
