...
<style>
  ...
  .status {
	...
	<head>
		<meta charset="utf-8">
		<title>Author Check status of story</title>
	</head>
	<main>        
  <div class="topnav">
...
<body>
	    ...
	    // output data of each row
		    while($row = mysqli_fetch_assoc($result)) {
	    		...
	    		$html = $html."<tr><td>".$row['STORY_ID']."</td><td>".$row['AUTHOR_NAME']."</td><td><a href='author_display_story.php?storyid=".$row['STORY_ID']."'>".$row['STORY_NAME']."</a>.</td><td>".$row['STORY_GENRE']."</td><td>".$row['AGE_GROUP']."</td><td>".$row['STORY_STATUS']."</td></tr>";
		...
		echo $html;
        echo "<a href=\"author_view.php\">GO BACK to Author View</a>";
		mysqli_close($conn);
